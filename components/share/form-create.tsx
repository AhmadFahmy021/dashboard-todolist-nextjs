...
export default function FormCreateShareList({id}:{id: string | any}){
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Create Successfully",{
                description: "Data berhasil disimpan ke server.",
                position: "top-right",
                duration: 1200
            });
            setTitle("")
            setDescription("")
            setTimeout(()=>{
                router.push(`/dashboard/shared/todo/list/${id}`)
            }, 1500)
        } catch (error: any) {
        ...
        <form onSubmit={handleSubmit}  className="grid gap-4">
                ...
                <div className="grid lg:grid-cols-10 md:grid-cols-5 sm gap-5">
                    ...
                    <Button type="button" className="w-full bg-red-500 hover:bg-red-600" disabled={loading}>
                        <Link href={`/dashboard/todo/list/${id}`}>
                            Cancel
                        </Link>
                    </Button>
                </div>
